...
class Openpbs(AutotoolsPackage):
    ...
    depends_on('pkg-config', type='build') # needed for kerbauth, even when kerbauth is not used
    ...
    depends_on('python@3.5:3.7', type=('build', 'link', 'run'), when='@20:')
    ...
    patch('python.patch', when='@:19')

    # compile and link fixes
    patch('gcc11.patch', when='@20:')
